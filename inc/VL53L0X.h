 ...
 * VL53L0X.h
 *
 * C API wrapper for the VL53L0X sensor driver
 */
...
typedef enum { VcselPeriodPreRange = 0, VcselPeriodFinalRange = 1 } vl53l0x_vcsel_period_t;

/* opaque device struct for instance data */
typedef struct {
  uint8_t address;
  uint8_t last_status;
  uint16_t io_timeout;
